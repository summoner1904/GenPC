...
def sign_up() -> Response | str:
    ...
    :return: Если все данные верны, сохраняет пользователя
     в БД, redirect(sign_in)
    """
    if request.method == "GET":
...
def cabinet() -> str:
    ...
    if request.method == "POST":
        if check_new_user_data(**dict(request.form)):
            ...
            flash(
                {"title": "Успешно!", "message": "Данные успешно изменены."},
                category="success",
            )
...
def configurator() -> str:
    ...
    if request.method == "POST":
        flash(
            {
                "title": "Успешно!",
                "message": "Ваш компьютер ждет вас в личном кабинете!",
            },
            category="success",
        )
...
def add_database() -> Response | str:
    ...
    Используется для добавления новых записей в базу данных.
     Доступно только администратору (логин serjasum).
    ...
    if request.method == "POST":
        if len(request.form.get("phone")) == NUMBER_LENGTH:
            ...
            flash(
                {
                    "title": "Успешно!",
                    "message": "Мы приняли вашу заявку. Ожидайте звонка.",
                },
                category="success",
            )
        else:
